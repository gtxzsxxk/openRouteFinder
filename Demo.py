...
读取后的整个图维护在nodeList中，因此在packData.py中，
我使用pickle库将nodeList对象序列化到二进制文件navRTE_as.dat中。
...
def SearchRoute(orig, dest):
    if (
        airport_maps.__contains__(orig) is False
        or airport_maps.__contains__(dest) is False
    ):
        return None
