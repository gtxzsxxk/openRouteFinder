...
logStringRedirect = ""
visitIpaddr = []
# In fact I never intend to record Ip addr and just do it for PV statistics
visitTime = []
rteGroup = []
...
def HttpResponse(sock: socket.socket, dat: str):
    head = "HTTP/1.1 200 OK\r\nContent-Type:text/html\r\nServer: gtxzsxxk/openRouteFinder\r\nAccess-Control-Allow-Origin: *\r\n\r\n"
    total = head + dat
...
def getHtmlLog():
    ...
    # </body></html>
    formatStr = (
        formatStr + "<h3>Total Page Visit:" + str(visitIpaddr.__len__()) + "</h3>"
    )
    formatStr = (
        formatStr
...
class SessionHandler(threading.Thread):
    ...
    def run(self):
        ...
        try:
            ...
            LogPrint(
                "线程({},{},开启线程统计{})==用户[{},{}]连接，请求{}   {}".format(
                    self.name,
                    self.thread_num,
                    threadNumber_statistic,
                    self.client_addr[0],
                    self.client_addr[1],
                    requestHead,
                    time.strftime("%m-%d %H:%M:%S", time.localtime()),
                )
            )  # /getRoute?from=ZGHA&to=ZJSY&valid=1111
            ...
            if command.__contains__("/getRoute") and para.split("&").__len__() == 3:
                ...
                if hasRequest == True:
                    ...
                    if data == None:
                        HttpResponse(
                            self.client_socket, "无法解析航路或查询的数据不存在。"
                        )
            ...
            traceback.print_exc()
            LogPrint(
                "线程({},当前线程数{},开启线程统计{})出错:{} 尝试回收当前线程   {}".format(
                    self.name,
                    self.thread_num,
                    threadNumber_statistic,
                    x,
            ...
            self.thread_num = len(threading.enumerate())
            LogPrint(
                "线程({},当前线程数{},开启线程统计{})退出 线程回收成功   {}".format(
                    self.name,
                    self.thread_num,
                    threadNumber_statistic,
                    time.strftime("%m-%d %H:%M:%S", time.localtime()),
...
def SearchRoute(orig, dest):
    if (
        airport_maps.__contains__(orig) == False
        or airport_maps.__contains__(dest) == False
    ):
        return None
    objsearch = RouteFinderLib.RTFCALC(
        airport_maps, PerloadNodeList, config.NAVDAT_CYCLE
    )
...
print(
    "Http Client Started.Visit http://127.0.0.1:%d/ for service." % config.LISTEN_PORT
)
